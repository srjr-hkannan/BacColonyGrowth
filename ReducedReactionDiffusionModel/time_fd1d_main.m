function [u, t_min] = time_fd1d_main(W,L,Ks,R,H,Q,u,vol, time_hour, t, h, prefactor, path)
...
%         disp(i);
        curr = convertindex(i,j,1,nL, nW, nR, h, R, H);
        A(curr,convertindex(i-1,j,1,nL, nW, nR, h, R, H)) = -Dp/(h^2);
        A(curr, convertindex(i,j-1,1,nL, nW, nR, h, R, H)) = -Dp/(h^2);
...
while growth_glu< glu || n_reached
    if growth_glu>= glu
        n_reached = false;
        sa = string(path)+'cs1000_c_hr_'+string(time_hour)+ ' 1du_iter'+ string(t_min)+' t= '+string(t)+' h='+string(h)+'.mat';
        save(sa,'u');
    end
    t_min= t_min+1;
    b = zeros(numk,1);
        ...
        for j=1:nL-1
            if j==1 && i<nR
                if i==1
                    ...
                    b_M(i,j)= b_M(i,j) + u_M(i+1,j) *Dm/(2*h^2)*(1);
                    b_M(i,j)= b_M(i,j)+u_M(i,j+1) *Dm/(2*h^2);
                    b_M(i,j)= b_M(i,j)+u_M(i,j)*((1/t)-(3)*Dm/(2*h^2));
                else
                    b_M(i,j)= u_M(i+1,j) *(1)*Dm/(2*h^2);
                    b_M(i,j)= b_M(i,j) +u_M(i-1,j) *(1)*Dm/(2*h^2);
            ...
            if i==1
                ...
                b_M(i,j)= b_M(i,j) + u_M(i+1,j) *Dm/(2*h^2)*(1);
                if j==nL-1 
                    b_M(i,j)= b_M(i,j)+u_M(i,j)*((1/t)-(2)*Dm/(2*h^2));
                else 
                    b_M(i,j)= b_M(i,j)+u_M(i,j+1) *Dm/(2*h^2);
                    b_M(i,j)= b_M(i,j)+u_M(i,j)*((1/t)-(3)*Dm/(2*h^2));
                end
            elseif i==nW-1
                b_M(i,j)= u_M(i-1,j) *Dm/(2*h^2)*(1);
                if j==1
                    b_M(i,j)= b_M(i,j)+u_M(i,j+1) *Dm/(2*h^2);
                    b_M(i,j)= b_M(i,j)+u_M(i,j)*((1/t)-(2)*Dm/(2*h^2));
                elseif j==nL-1
...
%                 b_A(curr)= b_A(curr)+u_A(convertindex(i+1,j,-1,nL, nW, nR, h, R, H)) *Dm/(2*h^2);
%                 if j==nL-1 
%                     b_A(curr)= b_A(curr)+u_A(curr)*((1/t)-2*Dm/(2*h^2));
%                 else 
%                     b_A(curr)= b_A(curr)+u_A(convertindex(i,j+1,-1,nL, nW, nR, h, R, H)) *Dm/(2*h^2);
...
%             l = lambda_c(lambda_s, Q0, q,  ui);
%             if j==0
%                 growth_glu = growth_glu+q*l*((ui)/(ui+1))*rho*t*(h^2)/2;
%             else
%                 growth_glu = growth_glu+q*l*((ui)/(ui+1))*rho*t*(h^2);
%             end
%             
%         end
%     end
...
%             end
%             if j==0
%                 maintain = maintain+alpha*m*t*(h^2)/2;
%             else
%                 maintain = maintain+alpha*m*t*(h^2);
%             end
%         end
%     end
%     maintain = maintain*R/W;
    grow = 0;
    for i = 1:nR-1
        r = t*Dm;
        interface_index = convertindex(i,0,1,nL, nW, nR, h, R, H);
        index = convertindex(i,1,-1,nL, nW, nR, h, R, H);
        grow = grow + r*(u(index) - u(interface_index));
        if i==1
            grow = grow- + (r/2)*(u(index) - u(interface_index));
        end
        if i==nR-1
            grow = grow + (R-(nR-0.5)*h)*(u(index) - u(interface_index))/h;
        end
    end
    growth_glu = growth_glu + grow;
...
sa = string(path) + 'cs1000_c_hr_'+string(time_hour)+ ' 1du_'+ string(2)+' t= '+string(t)+' h='+string(h)+'.mat';
save(sa,'u');
% sa = '/Users/dge/Desktop/colony/code/colony files/time/deep L/cs500_c_R= '+string(R)+' with_ace_1duA'+ ' t= '+string(t)+' h='+string(h)+'.mat';
% save(sa,'u_A');
